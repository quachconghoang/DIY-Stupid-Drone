...
//    cout << m_desc.sizes() << endl; //256 x pts
//    cout << m_desc.scalar_type() << endl;
//    m_desc = m_desc.transpose(0,1);
    ...
    cv::Mat desc = temp.t();
//    cout << desc.size().width << " - " << desc.size().height;
//    for (int i=0;i<m_desc.size(0);i++){
//        for (int j = 0; j < m_desc.size(1); ++j) {
//            cout << m_desc[i][j] - desc.at<float>(j,i) << "-";
//        }
//    }

//    for (int i = 0; i < kps.size(); ++i) {
//        cout << kps[i].pt << ": " << kps[i].response << "\n" << desc.row(i) << "\n";
//    }
