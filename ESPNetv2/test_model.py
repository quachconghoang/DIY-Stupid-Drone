...
# _data_dir = '/home/hoangqc/DATASETS/Kitti/Odometry/dataset/sequences/00/image_2'
_data_dir = '/home/hoangqc/DATASETS/CITYSCAPES/leftImg8bit/train/aachen'
_classes = 20
_model_s = 1.5
_pretrained = './pretrained_models/espnetv2_segmentation_s_1.5.pth'
_inWidth = 1024
_inHeight = 512



image_list = glob.glob(_data_dir + os.sep + '*.' + 'png')
modelA = net.EESPNet_Seg(classes=_classes, s=_model_s)

if not os.path.isfile(_pretrained):
    print('Pre-trained model file does not exist. Please check ./pretrained_models folder')
    exit(-1)
modelA = nn.DataParallel(modelA)
modelA.load_state_dict(torch.load(_pretrained))
modelA = modelA.cuda()
...
mean_kitty = [90.97373879999999, 95.31948195000001, 96.00120150000001] #[0.35675976 *255, 0.37380189 *255, 0.3764753 *255]
std_kitty =  [81.76560975, 81.8521083, 82.4295762] #[0.32064945 *255, 0.32098866 *255, 0.32325324 *255]

# imgName = image_list[0]
# img = cv2.imread(imgName)
# img = cv2.resize(img, (_inWidth, _inHeight))
# img_orig = np.copy(img)
#
# img = img.astype(np.float32)
# for j in range(3):
...
# img_tensor = torch.unsqueeze(img_tensor, 0)  # add a batch dimension
# img_tensor = img_tensor.cuda()
# img_out = modelA(img_tensor)
# classMap_numpy = img_out[0].max(0)[1].byte().cpu().data.numpy()
# classMap_numpy = cv2.resize(classMap_numpy, (_inWidth, _inHeight), interpolation=cv2.INTER_NEAREST)
# classMap_numpy_color = np.zeros((img.shape[1], img.shape[2], img.shape[0]), dtype=np.uint8)
#
# for idx in range(len(pallete)):
#     [r, g, b] = pallete[idx]
